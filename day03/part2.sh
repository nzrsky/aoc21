...
# 0 bits (5) than 1 bits (7), so keep only the 5 numbers with a 0 in the first position: 00100, 01111, 00111, 00010, and 01010.
...
// curl -sS -b "session=$AOC_SSID" https://adventofcode.com/2021/day/3/input > input

import Foundation

extension Int {
    func bit(at i: Int) -> Int { Int((self & (1 << i)) >> i) }
}

extension Array where Element == Int {
    func mostCommonBit(at i: Int) -> Int {
        let ones = reduce(into: 0) { $0 += $1.bit(at: i) }
        return ones >= (count - ones) ? 1 : 0
    }

    func subarrayWithCommonBit(at i: Int, _ comparator: (Int, Int) -> Bool) -> [Element] {
        let most = mostCommonBit(at: i)
        return filter { comparator($0.bit(at: i), most) }
    }
}

func solve(_ nums: [Int], i: Int, _ cmp: (Int, Int) -> Bool) -> Int {
    return nums.count > 1 && i >= 0 ? solve(nums.subarrayWithCommonBit(at: i-1, cmp), i: i-1, cmp) : (nums.first ?? 0)
}

var nums: [Int] = []
try! String(contentsOfFile: CommandLine.arguments.dropFirst().first!).enumerateLines { line, stop in
    nums.append(Int(line, radix: 2) ?? 0)
}

let bitWidth = Int.bitWidth - (nums.map { $0.leadingZeroBitCount }.min() ?? 0)
let o2 = solve(nums, i: bitWidth, ==)
let co2 = solve(nums, i: bitWidth, !=)
print(o2 * co2)