...
# Q: What is the power consumption of the submarine? (Be sure to represent your answer in decimal, not binary.)
...
try! String(contentsOfFile: CommandLine.arguments.dropFirst().first!).enumerateLines { line, stop in
    sums = zip(sums ?? [Int](repeating: 0, count: line.count), line.map { $0.wholeNumberValue! }).map { $0 + $1 }
    n += 1
}

let g = Int((sums ?? []).map { $0 > (n - $0) ? "1" : "0" }.joined(), radix: 2)!
let e = Int((sums ?? []).map { $0 > (n - $0) ? "0" : "1" }.joined(), radix: 2)!

print(e * g)


// var sums: [Int] = [], n: Int = 0
// try! String(contentsOfFile: CommandLine.arguments.dropFirst().first!).enumerateLines { line, stop in
//     let bits = line.map { $0.wholeNumberValue ?? 0 }
//     let base = !sums.isEmpty ? sums : [Int](repeating: 0, count: bits.count)
//     sums = zip(base, bits).map { $0 + $1 }
//     n += 1
// }
//
// let tmp = sums.map { $0 > (n - $0) ? "1" : "0" }.joined()
//
// let g = UInt(tmp, radix: 2) ?? 0
// let e = ~g & UInt.max >> (UInt.bitWidth - tmp.count)
